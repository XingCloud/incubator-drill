...
$CLASS = "org.apache.hadoop.hbase.regionserver.ScannerPerformanceTest" ;
...
chomp($CP);
chdir($libDir);
open (LOG,"<", `$JAVA_HOME/bin/java $JVMFLAGS $SP -cp $CP $CLASS $ARGV[0] $ARGV[1] $ARGV[2]  $ARGV[3] $ARGV[4]`) ;
while($line = <LOG>){
	print $line ;
}
