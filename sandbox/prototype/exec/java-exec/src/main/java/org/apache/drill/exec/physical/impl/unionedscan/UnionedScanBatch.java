...
public class UnionedScanBatch extends AbstractRecordBatch<UnionedScanPOP> implements RecordBatch {
  ...
  final List<VectorWrapper<?>> vectors = Lists.newLinkedList();
  final Map<MaterializedField, VectorWrapper<?>> fieldVectorMap = Maps.newHashMap();
  private WrapperHolder holder = new WrapperHolder(vectors);
  ...
  public UnionedScanBatch(FragmentContext context, UnionedScanPOP popConfig) throws ExecutionSetupException {
    super(popConfig, context);
    this.context = context;
    this.originalEntries  = popConfig.getEntries();
    sortEntries();
    checkEntryOverlap();
    initReader();
  }
  ...
  private void releaseCache() {
    ...
    cached.clear();
    if(vectors!=null){
      for (VectorWrapper<?> valueVector : vectors) {
        valueVector.release();
      }
      vectors.clear();      
    }
  ...
  private void doCache(RecordBatch.IterOutcome outcome, List<CachedFrame> cache) {
    ...
    ArrayList<VectorWrapper<?>> cachedVectors = new ArrayList<>();
    for (VectorWrapper<?> v : vectors) {
      if(v.getField().getName().equals(UNION_MARKER_VECTOR_NAME)){
        v.release();
      }else{
        cachedVectors.add(v.cloneAndTransfer());
      }
    }
  ...
  private class Mutator implements OutputMutator {
    ...
    public void removeField(MaterializedField field) throws SchemaChangeException {
      schemaChanged();
      VectorWrapper<?> vector = fieldVectorMap.remove(field);
    ...
    public void addField(ValueVector vector) {
      SimpleVectorWrapper<ValueVector> wrapper = new SimpleVectorWrapper<ValueVector>(vector);
      vectors.add(wrapper);
      fieldVectorMap.put(wrapper.getField(), wrapper);
      if(!vector.getField().getName().equals(UNION_MARKER_VECTOR_NAME)){
    ...
    public void removeAllFields() {
      ...
      for (int i = 0; i < vectors.size(); i++) {
        VectorWrapper<?> valueVector = vectors.get(i);
        valueVector.release();
      }
      vectors.clear();
      builder = BatchSchema.newBuilder();
    }
  ...
  public static class CachedFrame{
     ...
     BatchSchema schema;
     private int recordCount;
     private List<VectorWrapper<?>> vectors;
     private RecordBatch.IterOutcome outcome;
     private FragmentContext context;
 
     private WrapperHolder holder;
     public CachedFrame(BatchSchema schema,
                                   int recordCount,
                                   List<VectorWrapper<?>> vectors,
       ...
       this.context = context;
       this.holder = new WrapperHolder(vectors);
     }
    ...
    public WrapperHolder getHolder() {
      return holder;
    }
    ...
    public List<VectorWrapper<?>> getVectors() {
      return vectors;
    }
  ...
  public class UnionedScanSplitBatch implements RecordBatch{
    ...
    public TypedFieldId getValueVectorId(SchemaPath path) {
      switch (scanMode){
        case cached:
          return currentCachedData.getHolder().getValueVector(path);
        case direct:
    ...
    public VectorWrapper<?> getValueAccessorById(int fieldId, Class<?> clazz) {
      switch (scanMode){
        case cached:
          return currentCachedData.getHolder().getValueVector(fieldId, clazz);
        case direct:
            ...
            switch (outcome) {
              ...
              case OK_NEW_SCHEMA:
                ...
                if(logger.isDebugEnabled()){
                  Iterator<VectorWrapper<?>> it;
                  for (it = passEntryID(vectors.iterator()); it.hasNext();) {
                    VectorWrapper<?> vector = it.next();
                    logger.debug("vector on direct mode's next():{},{}...", vector.getValueVector().getAccessor().getValueCount(), vector.getValueVector().getAccessor().getObject(0));                    
                  }
                }
                return outcome;
            }
    ...
    public Iterator<VectorWrapper<?>> iterator() {
      ...
      switch(scanMode){
        case direct:
          if(logger.isDebugEnabled()){
            VectorWrapper<?> previous = null;
            for(Iterator<VectorWrapper<?>> it = passEntryID(vectors.iterator());it.hasNext();){
              VectorWrapper<?> vector = it.next();
              if(vector == previous){
                logger.warn("previous vector same as this!{}", vector.getField());
              }
              previous = vector;
              logger.debug("vector on direct mode's iterator():{},{}...", vector.getValueVector().getAccessor().getValueCount(), vector.getValueVector().getAccessor().getObject(0));
              try {
  ...
  private Iterator<VectorWrapper<?>> passEntryID(Iterator<VectorWrapper<?>> iterator) {
    return new PassEntryID(iterator);
  }
  
  static class PassEntryID implements Iterator<VectorWrapper<?>>{

    Iterator<VectorWrapper<?>> incoming;

    VectorWrapper<?> nextV;
    
    PassEntryID(Iterator<VectorWrapper<?>> incoming) {
    ...
    @Override
    public VectorWrapper<?> next() {
      VectorWrapper<?> tmp = nextV;
      nextNoEntryID();
      return tmp;
    }
  ...
  private int getEntryMark() {
    for(VectorWrapper<?> v:vectors){
      if(v.getField().getName().equals(UNION_MARKER_VECTOR_NAME)){
        return (Integer)v.getValueVector().getAccessor().getObject(0);        
      }
    }
    return -1;//impossible
  }
  ...
  static class WrapperHolder {
    ...
    public WrapperHolder(List<VectorWrapper<?>> vectors) {
      super();
      this.vectors = vectors;
    }
    ...
    public TypedFieldId getValueVector(SchemaPath path) {
      for (int i = 0; i < vectors.size(); i++) {
        VectorWrapper<?> vv = vectors.get(i);
    ...
    public <T extends VectorWrapper<?>> T getValueVector(int fieldId, Class<?> clazz) {
      VectorWrapper<?> v = vectors.get(fieldId);
      assert v != null;
