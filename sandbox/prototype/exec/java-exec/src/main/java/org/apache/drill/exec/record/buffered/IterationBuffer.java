...
public class IterationBuffer {
  ...
  private void doTransfer(RecordBatch.IterOutcome outcome) {
    ...
    List<VectorWrapper<? extends ValueVector>> vectors = new ArrayList<VectorWrapper<? extends ValueVector>>();
    for (VectorWrapper<?> v : head) {
      vectors.add(v.cloneAndTransfer());
    }
    /*
  ...
  public class BufferedStackFrameImpl implements BufferedStackFrame {
    ...
    private List<VectorWrapper<? extends ValueVector>> vectors;
    ...
    public BufferedStackFrameImpl(BatchSchema schema,
                                  ...
                                  List<VectorWrapper<? extends ValueVector>> vectors,
    ...
    @Override
    public List<VectorWrapper<? extends ValueVector>> getVectors() {
      return vectors;
    }
  ...
  public class StackFrameDelegate implements BufferedStackFrame {
    ...
    List<VectorWrapper<? extends ValueVector>> mirroredVectors;
      ...
      switch (delegate.getOutcome()) {
        case OK_NEW_SCHEMA:
          ...
          if (this.mirroredVectors != null) {
            for (VectorWrapper v : mirroredVectors) {
              v.release();
            }
          }
          this.mirroredVectors = new ArrayList<>();
          if (delegate.getVectors() != null) {
            for (VectorWrapper v : delegate.getVectors()) {
              TransferPair tp = v.getValueVector().getTransferPair();
              tp.mirror();
              mirroredVectors.add(new SimpleVectorWrapper(tp.getTo()));
            }
          }
          break;
        case OK:
          List<VectorWrapper<? extends ValueVector>> bufferedVectors = delegate.getVectors();
          for (int i = 0; i < mirroredVectors.size(); i++) {
            VectorWrapper mirrored = mirroredVectors.get(i);
            VectorWrapper buffered = bufferedVectors.get(i);
            buffered.getValueVector().getMutator().transferTo(mirrored.getValueVector(), false);
          }
          break;
        default:
          break;
