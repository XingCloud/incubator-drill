 ...
 * Time: 10:57 AM
 */
public class JoinBatch extends AbstractRecordBatch<JoinPOP> {
...
//  private BatchSchema batchSchema;
  ...
  private List<List<VectorWrapper<?>>> leftIncomings;
  ...
  private List<VectorWrapper<?>> rightVectors;
  ...
  public void setupEvals() {
    JoinCondition condition = popConfig.getConditoin();
  ...
  private void clearLeft() {
    for (List<VectorWrapper<?>> vectors : leftIncomings) {
  ...
  private void clearRight() {
    for (VectorWrapper<?> v : rightVectors) {
      v.release();
    }
  ...
  public static VectorWrapper<?> getVector(MaterializedField f, List<VectorWrapper<?>> vectors) {
    for (VectorWrapper<?> vector : vectors) {
    ...
    public void upstream() {
      ...
      for (MaterializedField f : leftSchema) {
        ...
        for (int i = 0; i < recordCount; i++) {
          ...
          if(indexes[0] != cursor){
            cursor = indexes[0] ;
            current = getVector(f, leftIncomings.get(indexes[0])).getValueVector().getAccessor() ;
          }
      ...
      for (MaterializedField f : rightIncoming.getSchema()) {
        ...
        AllocationHelper.allocate(out, recordCount, 8);
        outMutator = out.getMutator();
        ValueVector.Accessor accessor = getVector(f, rightVectors).getValueVector().getAccessor();
        for (int i = 0; i < recordCount; i++) {
          int[] indexes = outRecords.get(i);
