...
public class TestUtils {
  ...
  /**
   * @param resourcePath path for json plan file <<<<<<< HEAD
  ...
  public static Collection<RunOutcome> getOutcome(DrillConfig config, String resourcePath) throws IOException {
    LogicalPlan plan = LogicalPlan
      .parse(config, Files.toString(FileUtils.getResourceAsFile(resourcePath), Charsets.UTF_8));
  ...
  public static List<UnbackedRecord> getResultAsUnbackedRecords(String resourcePath) throws Exception {
    ...
    Collection<RunOutcome> outcomes = getOutcome(config, resourcePath);
    if (outcomes.size() != 1)
      throw new Exception("Only supports logical plans that provide a single outcome.");
    RunOutcome out = outcomes.iterator().next();
    switch (out.outcome) {
      case CANCELED:
      case FAILED:
        if (out.exception != null)
          throw (Exception) out.exception;
    ...
    while ((o = q.poll()) != null) {
      if (o instanceof OutcomeType)
        continue;
      if (!(o instanceof UnbackedRecord))
        throw new Exception(String.format(
          "This method only works when the QueueRSE is configured to use RECORD encoding.  One of the queue objects was of type %s",
