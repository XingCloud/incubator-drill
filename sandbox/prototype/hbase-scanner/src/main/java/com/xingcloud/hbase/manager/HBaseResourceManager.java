...
public class HBaseResourceManager {
    ...
    public HTable getTable(byte[] tableName) throws IOException {
        HTableInterface itable=pool.getTable(tableName);
        HTablePool.PooledHTable ptable=(HTablePool.PooledHTable)itable;
        return (HTable) ptable.getWrappedTable();
    ...
    public HTable getTable(String tableName) throws IOException {
        ...
        /*HTable htable = null;
        try {
            //htable = (HTable) ((HTablePool.PooledHTable)pool.getTable(tableName)).getWrappedTable();
        } catch (Exception e) {
